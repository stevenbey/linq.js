...
var grouped = groupByTest.groupBy(v => v.x);
...
Assert.equals(groupByTest.groupBy(v => v.x, v => v.a)[0][0], "abd", "groupBy: with key selector and element selector");
Assert.equals(groupByTest.groupBy(v => v.x, v => v.a, (k, a) => ({ value: k * a.length }))[0].value, 2, "groupBy: with key selector, element selector and result selector");
Assert.equals(groupByTest.groupBy(v => ({ z: v.z, b: v.b })).length, 2, "groupBy: with complex key");